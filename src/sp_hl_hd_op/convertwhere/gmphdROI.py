...
class GMPHD(object):
    ...
    def __init__(self, birthgmm, survival, detection, f, q, h, r, p0, clutter, merge_thresh, det_thresh=0.3):
        ...
        self.prev_measures =  np.zeros((8,1))
    ...
    def prune(self, truncation_thresh=1e-4, max_components=25):
        ...
        while len(I) > 0:
            ...
            for index, i in enumerate(I):
                temp = np.dot((i.mean - I[j].mean).T, np.linalg.inv(i.cov+np.eye(8)*0.00001))
                mah_dist = np.float64(np.dot(temp, (i.mean - I[j].mean)))
                mindist = np.linalg.norm(i.mean[:4:2]-I[j].mean[:4:2])       
                #print '==cov', i.cov         
                #print 'mah_dist', mah_dist
                #print 'min_dist', mindist
                #if mindist <= self.merge_thresh:   
                self.merge_thresh = 50   
                if (i.id != I[j].id and mindist <= self.merge_thresh) or (i.id == I[j].id and mindist <= self.merge_thresh*5):      
                #if mah_dist <= 3:#self.merge_thresh:
                    L.append(i)
    ...
    def create_birth(self, measures):
        ...
        for i in np.ndindex(measures.shape[1]):
            ...
            for j, comp in enumerate(self.gm):
                #print 'comp', comp
                if np.linalg.norm(z[:4]-comp.mean[:4]) < mindist:                
                    mindist = np.linalg.norm(z[:4]-comp.mean[:4])
