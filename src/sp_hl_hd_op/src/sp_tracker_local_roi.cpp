...
#include <pub_msgs/roilocation_msgs.h>
...
float SN_KINECT_OFFSET_X = 0.0;
float SN_KINECT_OFFSET_Y = 0.0;
float SN_KINECT_OFFSET_Z = 0.0;

float SN_KINECT_OFFSET_RPY_R 	= 0.0;
float SN_KINECT_OFFSET_RPY_P 	= 0.0;
float SN_KINECT_OFFSET_RPY_Y 	= 0.0;
cv::Mat depth_image_roi(480, 640, CV_16UC1);
...
ros::Publisher  pub;
ros::Publisher  pub_roi_location;
image_transport::Publisher img_pub;
image_transport::Publisher depthroiimage;
...
static const char TOPIC_NAME[] = "/sn_kinect2/rgb/image_color";
...
void publishTransforms(const std::string& frame_id) 
{
	...
	pub_msgs::roilocation_msgs roi_location_p;
	...
	for (int i = 0; i < users_count; ++i)
	{
		...
		//if (!g_UserGenerator.GetSkeletonCap().IsTracking(user))
			...
			// set (X,Y,Z) as CoM (center of mass)
			//X[users[i]] = (float)-p.X;
			//Y[users[i]] = (float)p.Y;
			//Z[users[i]] = (float)p.Z;
		//	continue;
		...
		// Get "p" ,the Center of Mass(CoM) of a user // unit = mm
		g_UserGenerator.GetCoM(users[i],p);
...
*/
		...
		// Converting to real world is Not used
...
//		sprintf(pos_text, "x before: %.2f, y before : %.2f, z before: %.2f", X[users[i]], Y[users[i]], Z[users[i]]);
		...
		// 2. print after converting to realworld
		//printf(" After projection : (x,y,z)   %.1f %.1f %.1f\n",X[users[i]],Y[users[i]],Z[users[i]]);
		//printf("SN_KINECT2_skel  %.1f %.1f %.1f\n",p_Head.X*1000.0,p_Head.Y*1000.0,p_Head.Z*1000.0);
		// ### head data via skeletion calculation
		//ROS_WARN("%.1f %.1f %.1f",p_Head.X*1000.0,p_Head.Y*1000.0,p_Head.Z*1000.0);
...
//	printf("a: %hu\n", depthPixels);
	...
	IplImage* imagePixels_mask_publish;
	imagePixels = cvCreateImage(cvSize(640,480),8,3);
	imagePixels_mask_publish = cvCreateImage(cvSize(640,480),8,1);
//	for(int ii=0;ii<480*640;ii++){
//		roi_location_p.roi_location.push_back(0);
//	}
...
//			imagePixels_mask_publish->imageData[(y*imagePixels_mask_publish->widthStep) + x*3+0] = 0;
//			imagePixels_mask_publish->imageData[(y*imagePixels_mask_publish->widthStep) + x*3+1] = 0;
//			imagePixels_mask_publish->imageData[(y*imagePixels_mask_publish->widthStep) + x*3+2] = 0;
			if (depthPixels[ii] > 0)
			{
//				printf("depthPixels[ii]: %hu\n", depthPixels[ii]);				
...
//				imagePixels_mask_publish->imageData[(y*imagePixels_mask_publish->widthStep) + x*3+0] = Colors[depthPixels[ii]%15][0];
//				imagePixels_mask_publish->imageData[(y*imagePixels_mask_publish->widthStep) + x*3+1] = Colors[depthPixels[ii]%15][1];
//				imagePixels_mask_publish->imageData[(y*imagePixels_mask_publish->widthStep) + x*3+2] = Colors[depthPixels[ii]%15][2];
...
//				depth_image_roi.at<unsigned short>(y,x)=0; 
	...
	cvCvtColor(imagePixels,imagePixels_mask_publish,CV_RGB2GRAY);
//	threshold(grayscaleMat, binaryMat, 100, 255, CV_THRESH_BINARY);

//	imagePixels_mask_publish=imagePixels
	int cnt=0;
	...
	for(int i=0; i<15; i++)
	{
		if((u_rx[i] - u_lx[i]) > 0 && (u_rx[i] - u_lx[i]) <= 640 && (u_ry[i] - u_ly[i]) > 0 && (u_ry[i] - u_ly[i]) <= 480)
		{
			...
			tf::Vector3 v1,v2;
			m1.setEulerZYX(SN_KINECT_OFFSET_RPY_Y, SN_KINECT_OFFSET_RPY_P, SN_KINECT_OFFSET_RPY_R);
			//float f = 525.5;
			
			// pixels : (640x480)
			// FOV : (62(deg),48.6(deg))
			// 1/f = atand(31)/320 = atand(24.3)/240 : focal length
			float f = 532.6 ;
			...
			// Head point : (X,Y,Z)
			// Get (x_pixel,y_pixel) from roi
			x_pixel = ((float)(320. - (float)(u_lx[i]+abs(u_rx[i] - u_lx[i])/2.))) ;
			y_pixel = ((float)(240. - u_ly[i])) ;

			// Update Z from CoM to head point
			Z[i] = ( z_com + y_com*tan(SN_KINECT_OFFSET_RPY_R) )/( 1 + y_pixel*tan(SN_KINECT_OFFSET_RPY_R)/f ) ;
			//X[i] = x_pixel*Z[i]/f ;
			X[i] = x_com ;  
			Y[i] = y_pixel*Z[i]/f ;
			...
			// Transform body frame(kinect) to global frame
			v2 = m1*v1 ;

			CvPoint pt1 = {u_lx[i], u_ly[i]};
			CvPoint pt2 = {u_rx[i], u_ry[i]};

			// Set (x,y,z) at the origin
			my_point.x = ((float)v2.getX() + SN_KINECT_OFFSET_X)*0.001;
			my_point.y = ((float)v2.getY() + SN_KINECT_OFFSET_Y)*0.001;	
			my_point.z = ((float)v2.getZ() + SN_KINECT_OFFSET_Z)*0.001;

			//ROS_ERROR("i:%d, %d, %d, %d, %d", i, u_lx[i], u_ly[i], u_rx[i], u_ry[i]);
			
			//printf(" CoM (x,y,z) : %.1f, %.1f, %.1f \n", x_com ,y_com,z_com);
			//ROS_ERROR("%.1f, %.1f, %.1f", X[i], Y[i], Z[i]);
...
//			}
			...
			if(my_point.z!=0 and my_roi.width*my_roi.height>200){

				g_track.cam_id.push_back(my_str);
				roi_location_p.cam_id.push_back(my_str);
...
//	for(int x=0;x<480*640;x++){
//		roi_location_p.roi_location.push_back(0);
//	}
//	int index =0;
//	
//	cv::Mat mat_image=cv::cvarrToMat(imagePixels_mask_publish); // image is Iplimage
//	unsigned char *input = (unsigned char*)(mat_image.data);

//	for(int i = 0;i < mat_image.rows ;i++){
//		    for(int j = 0;j < mat_image.cols ;j++,index++){
//				if (input[mat_image.step * i + j]!=0){
////					roi_location_p.roi_location.push_back(i);
////					roi_location_p.roi_location.push_back(j);
//					roi_location_p.roi_location[index]=1;
//				}
//		    }
//		}
	...
	for(int i = 0;i < 480 ;i++){
		    for(int j = 0;j < 640 ;j++){
				if (CV_IMAGE_ELEM(imagePixels_mask_publish, uchar, i, j) != 0){
					roi_location_p.roi_location.push_back(i);
					roi_location_p.roi_location.push_back(j);
				}
		    }
...
//		    for(int j = 0;j < 640 ;j++){
//				if (CV_IMAGE_ELEM(imagePixels_mask_publish, uchar, i, j) == 0){
//					depth_image_roi.at<unsigned short>(i,j)=0; 	
//				}
//		    }
//		}

//publish detector1 for location 	
	g_track.total = cnt;
	g_track.header.stamp = ros::Time::now();
	g_track.header.frame_id = frame_id;
	pub.publish(g_track);
////publish ROI location
//	roi_location_p.header=g_track.header;
//	pub_roi_location.publish(roi_location_p);

////publish mask for ROI
//	cv::Mat mat_image=cv::cvarrToMat(imagePixels_mask_publish); // image is Iplimage
//	sensor_msgs::Image msg;
	std_msgs::Header header; // empty header
	header   = g_track.header;

    if(!depth_image_roi.empty()) {
		sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "16UC1", depth_image_roi).toImageMsg();
		depthroiimage.publish(msg);
    }
	...
	*/
	...
	cvReleaseImage(&imagePixels_mask_publish);
...
int main(int argc, char **argv) 
{
	...
	ros::NodeHandle pnh;
	string frame_id("/sn_kinect2_depth_frame");
	pnh.getParam("camera_frame_id", frame_id);
	//ros::Subscriber sub = pnh.subscribe("sn_kinect2/rgb/image_color", 10, imageCb_rgb);
	
	//subcrible the dpeht raw to make the depth with only roi
	ros::Subscriber image_sub_;
	image_sub_ = pnh.subscribe("/sn_kinect2/depth/image_raw", 2, &RetornoImagen);

	//publish location 
	pub = pnh.advertise<pub_msgs::where_msgs>("/sn_kinect/detector2", 8);

	////publish roi mask
	//	ros::NodeHandle pnh_roi;
	//	pnh_roi.getParam("camera_frame_id", frame_id);
	//	pub_roi_location = pnh_roi.advertise<pub_msgs::roilocation_msgs>("depth_mask/sn_kinect2", 10);

	//depth_roi_image publish
	ros::NodeHandle pnh_depth_roi;
	image_transport::ImageTransport it(pnh_depth_roi);
	depthroiimage = it.advertise("/sn_kinect2/depth/depth_roi", 2);

	
	ros::Rate r(30);
	std::vector<double> global_name;
	int cnt=0;
	ROS_ERROR("===== sp_tracker(%s) is ready =========", camera_name);
	...
	while (ros::ok()&& quit!='q') 
	{
		...
		{		
			...
			ROS_ERROR("TRACKER --------------------\n set %s PARAM [%.2f, %.2f, %.2f, %.2f, %.2f, %.2f]", camera_name, global_name[0],global_name[1],global_name[2],global_name[3],global_name[4],global_name[5]);
			
			SN_KINECT_OFFSET_X 	= global_name[0]*1000.0;
			SN_KINECT_OFFSET_Y 	= global_name[1]*1000.0;
			SN_KINECT_OFFSET_Z 	= global_name[2]*1000.0;
			SN_KINECT_OFFSET_RPY_R = global_name[3];
			SN_KINECT_OFFSET_RPY_P = global_name[4];
			SN_KINECT_OFFSET_RPY_Y = global_name[5];                    
			cnt = 0;
		}
