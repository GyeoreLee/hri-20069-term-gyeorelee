...
int action_judge(XnPoint3D p_Head, XnPoint3D p_Torso, XnPoint3D p_Sh_Left, XnPoint3D p_El_Left, XnPoint3D p_Ha_Left, XnPoint3D p_Sh_Right, XnPoint3D p_El_Right,XnPoint3D p_Ha_Right)
{
	...
	else if((fabs(p_Head.Y-p_Torso.Y)<0.25))			{ return 4; }	// sleep
...
//			cnt++;
	...
	// release image buffer when not using GUI
...
int main(int argc, char **argv) 
{
	...
	ROS_WARN("The following devices were found");
	int i = 1;
	...
	ROS_WARN("Choose device to open : ");
	...
	ROS_WARN("Production tree of the DepthGenerator created.\n");
	...
	int cnt=0;
	ROS_ERROR("============= TRACKER (%s) starts  =================",camera_name);
	...
	while (ros::ok()&& quit!='q') 
	{
		...
		{		
			...
			ROS_WARN("get %s PARAM [%.1f, %.1f, %.1f, %.1f, %.1f, %.1f]", camera_name, global_name[0],global_name[1],global_name[2],global_name[3],global_name[4],global_name[5]);
		...
		if(cnt==0)
			ROS_INFO("read a new frame: g_Context.WaitAndUpdateAll() is calling...");	
